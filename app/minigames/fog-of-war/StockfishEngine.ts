export type UciMove = {
  from: string;
  to: string;
  promotion?: "q" | "r" | "b" | "n";
};

export type EngineResult = {
  bestMove: string | null;
  scoreType: "mate" | "cp" | null;
  score: number | null;
  rawLine?: string;
};

// Put these in /public:
//   /public/stockfish-18-lite-single.js
//   /public/stockfish-18-lite-single.wasm
const ENGINE_WORKER_URL = "/stockfish-18-lite-single.js";

function normalizeMove(move: string) {
  return move.replace(/\s+/g, "").toLowerCase();
}

function parseScore(line: string): { scoreType: "mate" | "cp"; score: number } | null {
  const match = line.match(/score\s+(mate|cp)\s+(-?\d+)/i);
  if (!match) return null;

  return {
    scoreType: match[1] as "mate" | "cp",
    score: Number(match[2]),
  };
}

function isBrowserWorkerAvailable() {
  return typeof window !== "undefined" && typeof Worker !== "undefined";
}

async function runEngine(fen: string, depth: number, timeoutMs: number): Promise<EngineResult> {
  if (!isBrowserWorkerAvailable()) {
    return {
      bestMove: null,
      scoreType: null,
      score: null,
    };
  }

  return await new Promise<EngineResult>((resolve) => {
    let worker: Worker | null = null;
    let finished = false;
    let timeoutId: number | null = null;

    let latestScoreType: "mate" | "cp" | null = null;
    let latestScore: number | null = null;

    const cleanup = () => {
      if (timeoutId !== null) {
        window.clearTimeout(timeoutId);
        timeoutId = null;
      }

      if (worker) {
        try {
          worker.terminate();
        } catch {
          // ignore termination errors
        }
        worker = null;
      }
    };

    const finish = (result: EngineResult) => {
      if (finished) return;
      finished = true;
      cleanup();
      resolve(result);
    };

    try {
      worker = new Worker(ENGINE_WORKER_URL);

      timeoutId = window.setTimeout(() => {
        try {
          worker?.postMessage("stop");
        } catch {
          // ignore stop errors
        }

        finish({
          bestMove: null,
          scoreType: latestScoreType,
          score: latestScore,
        });
      }, timeoutMs);

      worker.onmessage = (event) => {
        const line = String(event.data ?? "").trim();

        if (line === "uciok") {
          worker?.postMessage("isready");
          return;
        }

        if (line === "readyok") {
          worker?.postMessage("ucinewgame");
          worker?.postMessage(`position fen ${fen}`);
          worker?.postMessage(`go depth ${depth}`);
          return;
        }

        if (line.startsWith("info")) {
          const parsed = parseScore(line);
          if (parsed) {
            latestScoreType = parsed.scoreType;
            latestScore = parsed.score;
          }
          return;
        }

        if (line.startsWith("bestmove ")) {
          const move = line.split(/\s+/)[1] ?? null;

          finish({
            bestMove: move && move !== "(none)" ? normalizeMove(move) : null,
            scoreType: latestScoreType,
            score: latestScore,
            rawLine: line,
          });
        }
      };

      worker.onerror = () => {
        finish({
          bestMove: null,
          scoreType: latestScoreType,
          score: latestScore,
        });
      };

      worker.postMessage("uci");
    } catch {
      finish({
        bestMove: null,
        scoreType: null,
        score: null,
      });
    }
  });
}

export async function getBestMove(
  fen: string,
  depth = 16,
  timeoutMs = 8000,
): Promise<EngineResult> {
  return runEngine(fen, depth, timeoutMs);
}

export async function analyzePosition(
  fen: string,
  depth = 18,
  timeoutMs = 9000,
): Promise<EngineResult> {
  return runEngine(fen, depth, timeoutMs);
}

export function parseUciMove(uci: string): UciMove | null {
  const cleaned = normalizeMove(uci);
  if (cleaned.length < 4) return null;

  const from = cleaned.slice(0, 2);
  const to = cleaned.slice(2, 4);

  const promotion =
    cleaned.length > 4 ? (cleaned[4] as UciMove["promotion"]) : undefined;

  if (promotion && !["q", "r", "b", "n"].includes(promotion)) {
    return null;
  }

  return promotion ? { from, to, promotion } : { from, to };
}

function matePliesToMoves(matePlies: number): number {
  return Math.ceil(Math.abs(matePlies) / 2);
}

export function preservesForcedMate(
  result: EngineResult,
  remainingMoves: number,
): boolean {
  if (result.scoreType !== "mate") return false;
  if (result.score == null) return false;

  const mateMoves = matePliesToMoves(result.score);
  return mateMoves <= remainingMoves;
}